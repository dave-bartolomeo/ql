...
class TranslatedSimpleDirectInitialization extends TranslatedDirectInitialization {
  ...
  override predicate hasInstruction(Opcode opcode, InstructionTag tag, IRType resultType) {
    ...
    resultType = getIRTypeForPRValue(getContext().getTargetType())
  }
  ...
  override predicate hasInstruction(Opcode opcode, InstructionTag tag, IRType resultType) {
    ...
    exists(int startIndex, int elementCount |
      ...
      (
        ...
        resultType = getIRTypeForGLValue(getElementType())
        or
...
abstract class TranslatedFieldInitialization extends TranslatedElement {
  ...
  override predicate hasInstruction(Opcode opcode, InstructionTag tag, IRType resultType) {
    ...
    resultType = getIRTypeForGLValue(field.getType())
  }
...
class TranslatedFieldValueInitialization extends TranslatedFieldInitialization,
  ...
  override predicate hasInstruction(Opcode opcode, InstructionTag tag, IRType resultType) {
    ...
    opcode instanceof Opcode::Constant and
    resultType = getIRTypeForPRValue(field.getType())
    or
    tag = getFieldDefaultValueStoreTag() and
    opcode instanceof Opcode::Store and
    resultType = getIRTypeForPRValue(field.getUnspecifiedType())
  }
...
class TranslatedElementValueInitialization extends TranslatedElementInitialization,
  ...
  private IRType getDefaultValueType() {
    if elementCount = 1 then
      result = getIRTypeForPRValue(getElementType())
    else
...
abstract class TranslatedBaseStructorCall extends TranslatedStructorCallFromStructor {
  ...
  final override predicate hasInstruction(Opcode opcode, InstructionTag tag, IRType resultType) {
    ...
    resultType = getIRTypeForGLValue(call.getTarget().getDeclaringType())
  }
